    ...
    private int prepareConfigs(final MixinEnvironment environment) {
        ...
        for (final MixinConfig config : this.pendingConfigs) {
            try {
                ...
                MixinTransformer.logger.error("Error encountered whilst initialising mixin config '" + config.getName() + "': " + message, ex2);
            }
        }
        for (final MixinConfig config : this.pendingConfigs) {
        ...
        for (final MixinConfig config : this.pendingConfigs) {
            try {
                ...
                final String message = ex2.getMessage();
                MixinTransformer.logger.error("Error encountered during mixin config postInit step'" + config.getName() + "': " + message, ex2);
            }
        }
